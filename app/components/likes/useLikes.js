...
const STORAGE_KEY = "likes.byAccount.v1";
...
export function LikesProvider({ children }) {
  ...
  const updateItems = useCallback(
    updater => {
      ...
      setItems(previous => {
        ...
        const changed =
          sanitised.length !== previous.length || sanitised.some((item, index) => item.id !== previous[index]?.id);
  ...
  const inLikes = useCallback(id => items.some(movie => movie.id === id), [items]);
  ...
  const value = useMemo(
    ...
    [items, inLikes, toggle, add, remove, isPersistent, owner],
  );
